...
router.get('/:id', async (req: AuthRequest, res: Response) => {
  const { data: client } = await supabase.from('clients').select('*').eq('id', (req as any).params.id).eq('user_id', (req as any).user!.id).single()
  if (!client) return res.status(404).json({ error: 'Client not found' })
  const { data: invoices } = await supabase.from('invoices').select('*').eq('client_id', (req as any).params.id).order('created_at', { ascending: false })
  const stats = {
...
router.put('/:id', async (req: AuthRequest, res: Response) => {
  const { data, error } = await supabase.from('clients').update((req as any).body).eq('id', (req as any).params.id).eq('user_id', (req as any).user!.id).select().single()
  if (error) return res.status(400).json({ error: error.message })
  return res.json(data)
})

router.delete('/:id', async (req: AuthRequest, res: Response) => {
  await supabase.from('clients').update({ is_archived: true }).eq('id', (req as any).params.id).eq('user_id', (req as any).user!.id)
