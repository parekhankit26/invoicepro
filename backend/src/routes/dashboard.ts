...
router.get('/overview', async (req: AuthRequest, res: Response) => {
  ...
  const isActuallyOverdue = (inv: any) => inv.status !== 'paid' && inv.status !== 'cancelled' && new Date(inv.due_date) < now
  const totalOverdue = invoices.filter(isActuallyOverdue).reduce((s, i) => s + i.total, 0)
  ...
  return res.json({
    ...
    status_breakdown: { draft: invoices.filter(i => i.status === 'draft').length, sent: invoices.filter(i => i.status === 'sent').length, pending: invoices.filter(i => i.status === 'pending').length, paid: invoices.filter(i => i.status === 'paid').length, overdue: invoices.filter(isActuallyOverdue).length },
