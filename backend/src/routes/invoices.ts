...
router.get('/', async (req: AuthRequest, res: Response) => {
  const { status, client_id, search, page = 1, limit = 20, sort = 'created_at', order = 'desc' } = req.query
  let query = supabase.from('invoices').select(`*, clients(id,name,email), invoice_items(*)`, { count: 'exact' }).eq('user_id', req.user!.id).order(sort as string, { ascending: order === 'asc' }).range((+page-1)*+limit, +page*+limit-1)
