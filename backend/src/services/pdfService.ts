...
export const pdfService = {
  async generateInvoicePDF(invoice: any): Promise<Buffer> {
    ...
    await page.setContent(generateHTML(invoice), { waitUntil: 'networkidle0' })
...
function fmt(n: number, currency = 'GBP') {
  const symbols: Record<string,string> = { GBP:'£',USD:'$',EUR:'€',CAD:'C$',AUD:'A$',CHF:'Fr',JPY:'¥' }
...
<style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:system-ui,sans-serif;color:#1e293b;font-size:13px}
.hdr{background:#0f172a;color:#fff;padding:36px 48px;display:flex;justify-content:space-between;align-items:start}
.hdr h1{font-size:22px;font-weight:700}.hdr .sub{color:#94a3b8;font-size:12px;margin-top:4px}
.inv-num{font-size:20px;font-weight:700;text-align:right}.inv-status{display:inline-block;padding:3px 10px;border-radius:20px;font-size:11px;font-weight:600;background:#22c55e;color:#fff;margin-top:6px;text-transform:uppercase}
.meta{display:grid;grid-template-columns:1fr 1fr 1fr;gap:24px;padding:28px 48px;background:#f8fafc;border-bottom:1px solid #e2e8f0}
.meta label{font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:0.6px;color:#94a3b8;display:block;margin-bottom:5px}
.content{padding:28px 48px}
table{width:100%;border-collapse:collapse;margin-bottom:20px}
thead tr{background:#0f172a;color:#fff}th{padding:9px 12px;text-align:left;font-size:11px;font-weight:600;text-transform:uppercase}
th:last-child{text-align:right}td{padding:11px 12px;border-bottom:1px solid #f1f5f9}td:last-child{text-align:right;font-weight:500}
...
.ttotal{display:flex;justify-content:space-between;padding:10px 0;font-size:16px;font-weight:700;color:#0f172a;border-top:2px solid #0f172a;margin-top:4px}
.notes{margin-top:24px;padding:16px;background:#f8fafc;border-left:3px solid #0f172a;border-radius:0 8px 8px 0;font-size:13px;color:#374151;line-height:1.6}
.footer{margin-top:32px;padding:16px 48px;background:#f8fafc;border-top:1px solid #e2e8f0;font-size:11px;color:#94a3b8;display:flex;justify-content:space-between;align-items:center}
</style></head><body>
<div class="hdr">
...
<div class="footer"><span>Thank you for your business · ${p.company_name||p.full_name||''}</span>${invoice.stripe_payment_link?`<span>Pay: ${invoice.stripe_payment_link}</span>`:''}</div>
</body></html>`
}
