            ...
            <form onSubmit={handleSubmit(d => saveMutation.mutate(d))}>
              <div className="modal-body">
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                  <div className="form-group"><label className="form-label">Category</label><select {...(register as any)('category')} className="form-select">{CATS.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                  <div className="form-group"><label className="form-label">Date</label><input {...(register as any)('date')} className="form-input" type="date" /></div>
                  <div className="form-group" style={{ gridColumn: '1/-1' }}><label className="form-label">Description *</label><input {...(register as any)('description', { required: true })} className="form-input" placeholder="e.g. Adobe Creative Cloud" /></div>
                  <div className="form-group"><label className="form-label">Amount *</label><input {...(register as any)('amount', { required: true, valueAsNumber: true })} className="form-input" type="number" min="0" step="0.01" /></div>
                  <div className="form-group"><label className="form-label">Currency</label><select {...(register as any)('currency')} className="form-select">{['GBP','USD','EUR','CAD'].map(c => <option key={c} value={c}>{c}</option>)}</select></div>
                  <div className="form-group"><label className="form-label">VAT amount</label><input {...(register as any)('tax_amount', { valueAsNumber: true })} className="form-input" type="number" min="0" step="0.01" /></div>
                  <div className="form-group"><label className="form-label">Client (if billable)</label><select {...(register as any)('client_id')} className="form-select"><option value="">No client</option>{(Array.isArray(clients) ? clients : []).map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
                  <div className="form-group" style={{ gridColumn: '1/-1', display: 'flex', alignItems: 'center', gap: 8 }}><input {...(register as any)('is_billable')} type="checkbox" id="billable" style={{ width: 'auto' }} /><label htmlFor="billable" className="form-label" style={{ margin: 0, cursor: 'pointer' }}>Billable to client</label></div>
                </div>
              </div>
